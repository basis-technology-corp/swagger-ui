function rosetteCust(keyExists) {
    ...
    */
    ...
    $('tbody.operation-params').children().filter(function() {
        var str = $(this).children('td.code').text();
        return $.inArray(str, ['profileId', 'debug', 'output']) > -1 ||
            str.startsWith('hidden');
    }).css('display', 'none');
    ////////////////////////
    ...
    $('form input[name=request]').each(function() {
        $(this).attr('accept', 'application/json');
    });
