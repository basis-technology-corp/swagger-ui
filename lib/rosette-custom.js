function rosetteCust() {
    ...
    var client = new ZeroClipboard($('.response-copy-button'));
    client.on('ready', function(event) {
        $(event.target).attr('title', 'Copy response to clipboard').tooltip();
    });
    client.on('copy', function (event) {
        var clipboard = event.clipboardData;
        clipboard.setData('text/plain', $(event.target).parent().parent().find('pre.json').text());
    });
    client.on('aftercopy', function(event) {
        $(event.target).attr('title', 'Copied!').tooltip('fixTitle').tooltip('show').attr('title', 'Copy response to clipboard').tooltip('fixTitle');
    });

    /* lxu - 20150519 - disable try it when no key - uncomment below only in 3scale
    if (!keyExists) {
        $('input.submit').each(function() { $(this).click(function() { return false; }) });
      $('input.submit').popover({'html': 'true', 'data-trigger': 'focus', 'trigger': 'click', 'placement': 'right', 'title': 'Hold Up!', 'content': 'These docs work with your API key.<br />Please <a href="/login">login</a> or <a href="/signup">create a free account</a> to enable your key.'});
    }
    */
